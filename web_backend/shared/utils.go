...
func GenerateWsToken(userId, userName string) (string, error) {
...
func GenerateInviteToken(senderId, receiverId string) (string, error) {
	// 获取USER_EXPIRATION_HOURS环境变量
	expirationHoursStr := os.Getenv("INVITION_EXPIRATION_HOURS")
	...
	if err != nil {
		// 如果转换失败，记录日志并设置默认值为24小时
		log.Println("Invalid INVITION_EXPIRATION_HOURS, using default value of 24 hours.")
		expirationHours = 24
	}
