...
func GetUserFriends(c *gin.Context) {
	...
	query := `
        SELECT ud.user_id, ud.nickname, ud.avatar_url
        FROM user_friends uf
        JOIN user_details ud ON uf.friend_id = ud.user_id
        WHERE uf.user_detail_id = ?
    `

	// Prepare the result struct to hold the friends' information
	var friends []struct {
		UserID    string `json:"user_detail_id"`
		Nickname  string `json:"nickname"`
		AvatarURL string `json:"avatar_url"`
	}
