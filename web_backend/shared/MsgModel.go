...
const (
	//好友添加申请
	FriendInvitationType = "friend"
	//用户邀请进群
	UserInvitationType = "user"
	//用户申请入群
	GroupApplicationType = "group"
)
...
func NewWsUserInvitionMessage(receiverId []string, senderId, msgContent, senderNickname, senderAvatarUrl, token string) WsMsg {
	header := WsHeader{
		SenderId:       senderId,
		ReceiverId:     receiverId,
		MsgContentType: MsgContentTypeText,
	...
	body :=
		WsBody{
			...
			Extra: map[string]interface{}{
				"invition_type":     FriendInvitationType,
				"sender_id":         senderId,
...
func NewWsGroupApplicationMessage(receiverId []string, senderId, msgContent, senderNickname, senderAvatarUrl, groupId, token string) WsMsg {
	header := WsHeader{
		SenderId:       senderId,
		ReceiverId:     receiverId,
		MsgContentType: MsgContentTypeText,
	...
	body := WsBody{
		MsgContent: msgContent,
		Extra: map[string]interface{}{
			"invition_type":     GroupApplicationType,
			"sender_id":         senderId,
			"sender_nickname":   senderNickname,
			"sender_avatar_url": senderAvatarUrl,
			"group_id":          groupId,
...
func NewWsGroupInvitionMessage(receiverId []string, senderId, msgContent, senderNickname, senderAvatarUrl, group_name, groupAvatarUrl, groupId, token string) WsMsg {
	header := WsHeader{
		SenderId:       senderId,
		ReceiverId:     receiverId,
		MsgContentType: MsgContentTypeText,
	...
	body := WsBody{
		MsgContent: msgContent,
		Extra: map[string]interface{}{
			"invition_type":     UserInvitationType,
			"sender_id":         senderId,
			"sender_nickname":   senderNickname,
			"sender_avatar_url": senderAvatarUrl,
			"group_id":          groupId,
